...
- 冷却恢复由应用分配链路内联处理，不引入独立 worker 入口
- 运行面保持单程序：`uvicorn main:app --app-dir src`
- quota sync 继续移出 `v1` 范围，不保留伪实现入口

完成标准：

- [x] cooldown 恢复稳定运行
- [x] 不存在根目录独立 worker 入口
...
- `.env.example` 默认使用外部服务地址（示例为 `localhost`，可按环境覆盖）
